...
class MonodepthModel(object):
    ...
    # def __init__(self, params, mode, left, right, reuse_variables=None, model_index=0,model_pair=None):
    def __init__(self, params, mode, input_image, input_orig_image,label_image, target_pos, target_onehot_pos, reuse_variables=None, model_index=0, num_output=1):
        ...
        # self.input_image = tf.random_normal(input_image.get_shape(), mean=0, stddev=10/255) + input_image

        self.target_pos = target_pos
        # self.center_list = input_image_list
    ...
    def wrap_all_image(self, batch_img, batch_disp, batch_target_pos):
        '''
        :param batch_img: 输入的图片 （batch，h, w, c=27）
        :param batch_disp: disp网络输出的视差 (batch, h, w, c=1)
        :param batch_target_pos: 新视点的角度坐标，[batch][x1,y1]
        :return: 新合成的视点图像 （batch，h, w, c=27）
        '''
        with tf.variable_scope('images'):
            # assert batch_img.get_shape().as_list()[0] == batch_disp.get_shape().as_list()[0] == batch_target_pos.get_shape().as_list()[0]
            # 对每一个batch的同一位置的输入图像进行wrap
    ...
    def conv(self, x, num_out_layers, kernel_size, stride, activation_fn=tf.nn.elu):
        ...
        return slim.conv2d(p_x, num_out_layers, kernel_size, stride, 'VALID', activation_fn=activation_fn)#normalizer_fn=tf.contrib.layers.instance_norm
    ...
    def input_network(self, x, output_channel):
        input = x
        # with slim.arg_scope([slim.conv2d], normalizer_fn=slim.instance_norm):
        x = slim.conv2d(x, 64, 3, padding='same', activation_fn=tf.nn.elu)
        # c1 = tf.concat([c1, x], axis=3)

        x = slim.conv2d(x, 64, 3, padding='same', activation_fn=tf.nn.elu)
        # c2 = tf.concat([c2, c1, x], axis=3)
        x = slim.conv2d(x, 64, 3, padding='same', activation_fn=tf.nn.elu)
    ...
    def color_network(self, x, output_channel):
        ...
        # c3 = tf.concat([c3, c2, c1 ,x], axis=3)
        x = slim.conv2d(x, 32, 3, padding='same', activation_fn=tf.nn.elu)
        ...
        #         x = slim.conv2d(x, 32, 3, padding='same', activation_fn=tf.nn.elu)
        #         # c3 = tf.concat([c3, c2, c1 ,x], axis=3)
        #         x = slim.conv2d(x, 32, 3, padding='same')
        #
        #         x = slim.conv2d(x, 16, 3, padding='same')
        #
        #         x = slim.conv2d(x, output_channel, 3, padding='same')
        #         return x

    def build_color_net(self, input, disp):
        # color_net_input = tf.stop_gradient(tf.concat([input, disp, self.all_pos], axis=3))
    ...
    def build_losses(self):
        with tf.variable_scope('losses'):
            ...
            # wraped_image_loss_list = []
            # color_loss = [tf.reduce_mean(tf.abs(self.color_output[i] - self.label_image[i])) for i in
            #               range(self.num_output)]
            # self.color_loss = tf.add_n(color_loss)
            self.warped_image_loss = tf.add_n([tf.reduce_mean(tf.abs(self.wraped_image[:, :, :, i*3:(i+1)*3] - self.label_image))
                                               for i in range(9)])/9

            # inout_loss
            # self.input_loss = tf.reduce_mean(tf.square(self.denoise_input - self.input_label))

            # TOTAL LOSS
            # self.total_loss = (self.ssim_loss + self.l2_loss + self.warped_image_loss + 5 * self.input_loss) * 49
    ...
    def build_summaries(self):
        ...
        with tf.device('/cpu:0'):
            ...
            # tf.summary.image('img_center_input_adjust_tone', (self.input_image[:, :, :, 12:15]**0.5), collections=self.model_collection)
            ...
            # tf.summary.image('color_res', (tf.clip_by_value(1-tf.abs(self.color_net_res),0,1)), collections=self.model_collection)
            # tf.summary.image('img_center_pic', (self.center_pic), collections=self.model_collection)
            # tf.summary.image('img_center_to_side_est', (self.img_center_to_side_est[24]), collections=self.model_collection)
            ...
            # tf.summary.scalar('input_loss', self.input_loss, collections=self.model_collection)
            # tf.summary.scalar('color_loss', self.color_loss, collections=self.model_collection)
            # for i in range(0, self.num_output, 16):
            #     tf.summary.image('color_net_est_' + str(i), self.color_output[i], max_outputs=4,
