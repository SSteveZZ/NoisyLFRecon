...
-------------------------------------------------
   ...
   File Name    :  test_onehot_model
   Author       :  sjw
   Time         :  19-5-29 20:29
   Description  :  测试multitask的脚本文件
-------------------------------------------------
   Change Activity: 
                   19-5-29 20:29
-------------------------------------------------
...
from monodepth_dataloader_noise import *
...
parser.add_argument('--model_name',                type=str,   help='model name', default='TCW_test_set')
...
parser.add_argument('--data_path',                 type=str,   help='path to the data', default='/home/sjw/Desktop/extra_hard_disk/dataset/training/')#/home/sjw/Desktop/
parser.add_argument('--filenames_file',            type=str,   help='path to the filenames txt file', default='/home/sjw/Desktop/extra_hard_disk/dataset/flowers_train_set_49_allin.txt')
parser.add_argument('--test_file',                 type=str,   help='path to the test file',           default='/home/sjw/LF_reconstruction/utils/filenames/new_TCW_test_set_49.txt')
...
parser.add_argument('--output_directory',          type=str,   help='output directory for test disparities, if empty outputs to checkpoint folder', default='/media/sjw/Samsung_T5/LF/output')
...
parser.add_argument('--checkpoint_path',           type=str,   help='path to a specific checkpoint to load', default='/media/sjw/Samsung_T5/LF/ckpt/')#/home/sjw/tensorflow_temp/monodepth_multi/log/monodepth/multi_to_one/model-10
...
def test(params, step=0, sigma=0, mode=''):
    ...
    if args.checkpoint_path == '':
        ...
        restore_path = restore_path + 'model-{}'.format(step)
    ...
    for num in range(num_test_samples):
        ...
        # path_to_disp_est = output_directory + '/' + args.model_name + '/model_%d' % step + '/disp_est' + '/sample_%04d' % num
        # path_to_img_est = output_directory + '/' + args.model_name + '/model_%d' % step + '/est_img' + '/sample_%04d' % num
        # path_to_gt = output_directory + '/' + args.model_name + '/model_%d' % step + '/gt_img' + '/sample_%04d' % num
        # path_to_input = output_directory + '/' + args.model_name + '/model_%d' % step + '/inputt_img' + '/sample_%04d' % num
        ...
        with open(output_directory + '/' + args.model_name + '/psnr_ssim_rgb.txt', 'a') as fileobject:
            if num == 0:
                # title = '{:<20}{:<30}{:<30}{:<30}{:<30}\n'.format('step' + str(step), 'SSIM', 'PSNR', 'MSE', 'NRMSE')
...
def main(_):
    ...
    step_list = [110000]
    # gauss_list = [0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8,8.5,9,9.5,10]
    gauss_list = [0]
    # s_and_p_list = [0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009,0.01]
    s_and_p_list = [0]
    for i in step_list:
    ...
    # test(params, 70000, 0, mode='gauss')
    # test(params, 70000, 10, mode='gauss')
    # test(params, 70000, 0.01, mode='s&p')
